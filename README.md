...
Для запуска веб-приложения в файле .env необходимо определить следующие перменные:

REACT_APP_MAPBOX_API_KEY - ключ для работы карты (mapbox api)

REACT_APP_CURRENCY_API_URL - ссылка для запросов к api на получение актуального курса валют. Ответ должен возвращаться в формате JSON.

### В каталоге проекта вы можете запустить:

#### 1. yarn yarn webpack:dev
